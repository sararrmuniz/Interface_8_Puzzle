...
public final class Cromossomo {
    ...
    // Verificação exata da solução ótima
    int[] solucaoOtima = {1, 2, 3, 4, 5, 6, 7, 8, 0};
    if (Arrays.equals(tabuleiroAtual, solucaoOtima)) {
        this.fitness = 0.0;
        this.distancia = 0.0;
    ...
    int[] estadoAnterior = Arrays.copyOf(tabuleiro, tabuleiro.length);
    ...
    for (String movimento : movimentos) {
        ...
        switch (movimento.toLowerCase()) {
            case "cima":
                if (novaLinha > 0) novaPos = posVazia - 3;
                break;
            case "baixo":
                if (novaLinha < 2) novaPos = posVazia + 3;
                break;
            case "esquerda":
                if (novaColuna > 0) novaPos = posVazia - 1;
                break;
            case "direita":
                if (novaColuna < 2) novaPos = posVazia + 1;
                break;
        }

        if (novaPos != -1) {
            // Aplica o movimento temporariamente
            int[] novoTabuleiro = Arrays.copyOf(tabuleiro, tabuleiro.length);
            novoTabuleiro[posVazia] = novoTabuleiro[novaPos];
            novoTabuleiro[novaPos] = 0;

            // Verifica se o movimento alterou o tabuleiro
            if (!Arrays.equals(tabuleiro, novoTabuleiro)) {
    ...
    // Atualiza a lista de movimentos para conter apenas os válidos
    this.movimentos = movimentosValidos;
    return tabuleiro;
}
